            ...
            <v-col>
              <v-row>
                ...
                <v-col cols="2">
                  <span
                    class="delete"
                    @click.prevent="deleteSearchNote(note, index)"
                  >
...
export default {
  ...
  methods: {
    ...
    search(keyword) {
      ...
      console.log(keyword);
      // 제목과 내용에 대해서 검색
      let titleSearchResult = this.notes.filter((note) =>
        note.title.includes(keyword)
      );
      let textSearchResult = this.notes.filter((note) =>
        note.text.includes(keyword)
      );
      this.searchNotes = titleSearchResult.concat(textSearchResult);
      // 중복 제거
      this.searchNotes = this.searchNotes.filter((item, idx, array) => {
